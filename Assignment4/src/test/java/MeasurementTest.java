...
public class MeasurementTest {
    ...
    public void canConvertATextLineToAMeasurement() {
        ...
        checkMeasurement(measurement2, maastricht, 1975, 9, 14,
                ...
                1.4,
                0.6, 0.6);
        Measurement measurement3 = Measurement.fromLine("  380,19060205,   95,   36,   36,   77,   12,    5,   24,     ,     ,   ,  ,     ,   ,     ,     ,     ,   -1,   50,     ,     ,   -1  ,  -1  ,     ,10218,10251,     ,10196,     ,     ,     ,     ,     ,     ,   47,     ,     ,     ,     ,     ", stations);
        checkMeasurement(measurement3, maastricht, 1906, 2, 5,
                3.6, Double.NaN,
    ...
    private void checkMeasurement(Measurement measurement,
        ...
        assertNotNull(measurement, "Measurement was not parsed properly");
        ...
        assertEquals(fg, measurement.getAverageWindSpeed(), 1E-6, "FG Average wind speed not properly imported or processed");
        assertEquals(fxx, measurement.getMaxWindGust(), 1E-6, "FXX max wind gust not properly imported or processed");
        assertEquals(tg, measurement.getAverageTemperature(), 1E-6, "TG Average temperature not properly imported or processed");
        assertEquals(tn, measurement.getMinTemperature(), 1E-6, "TN minimum temperature not properly imported or processed");
        assertEquals(tx, measurement.getMaxTemperature(), 1E-6, "TX max temperature not properly imported or processed");
        assertEquals(sq, measurement.getSolarHours(), 1E-6, "SQ daily solar hours not properly imported or processed");
        assertEquals(rh, measurement.getPrecipitation(), 1E-6, "RH daily precipitation not properly imported or processed");
        assertEquals(rhx, measurement.getMaxHourlyPrecipitation(), 1E-6, "RHX max hourly precipitation not properly imported or processed");
    }
}